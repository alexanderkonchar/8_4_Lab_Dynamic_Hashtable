...
bool checkTest(const string &testName, elemType actual, elemType expected);
...
bool checkTest(const string &testName, elemType actual, elemType expected) {
    ...
    if (actual == expected) {
        ...
        cout << "***Failed test " << testName << " ** " << endl << "   output was " << actual << endl
             << "   output should have been " << expected << endl;
        return false;
    }
}